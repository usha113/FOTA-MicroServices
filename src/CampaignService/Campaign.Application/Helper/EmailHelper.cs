...
namespace Campaign.Application.Helper
{
    public class EmailHelper : IEmailHelper
    {
        ...
        public string toAddress = "usha.krishnan@wabco-auto.com";
        public string _userName = "";

        public async Task SendEmail(SmtpClient client, string toAddress, EmailType type, Campaign.Core.Entities.Campaign campaign)
        {
           // _userName = user;
            using (MailMessage mail = new MailMessage(new MailAddress(fromAddress), new MailAddress(toAddress))
            {
                ...
                Body = GenerateEmailContent(type, campaign),
                IsBodyHtml = true
       ...
       private string GenerateEmailSubject(EmailType type)
        {
            ...
            switch (type)
            {
                case EmailType.CampaignInitiated:
                    subject = "New Campaign Initiated";
                    break;
                case EmailType.CampaignApproved:
        ...
        private string GenerateEmailContent(EmailType type, Campaign.Core.Entities.Campaign campaign)
        {
            string template = "";
            switch(type)
            {
                case EmailType.CampaignInitiated:
                    template = GetCampaignInitiatedTemplate(campaign);
                    break;
                case EmailType.CampaignApproved:
                    template = AfterCampaignApprovedTemplate(campaign);
                    break;
        ...
        private string GetCampaignInitiatedTemplate(Campaign.Core.Entities.Campaign campaign)
        {
            string head = @"<head>
            ...
            return String.Format(@"<!doctype html>
												...
												<table width='100%' cellspacing='0' cellpadding='0' border='0' align='center'
													...
													<tr>
														<td style='font-size: 14px; color: #333333; text-align: left; font-family: Arial, Helvetica,  sans-serif; line-height: 24px; vertical-align: top; padding:20px 0 0 0; '
															width='99%'>
															<p style='mso-table-lspace:0;mso-table-rspace:0; margin:0;'>
																<span style='font-family: Arial, Helvetica, sans-serif '>New Campaign has been initiated by Iniator.Please find below the campaign details.Looking for your approval.Kindly click the link to approve the campaing.
                                                                </span>
															</p>
														</td>
													</tr>
                ...
                </html>", DateTime.Now.Date.ToString("MMM dd yyyy"), _userName, campaign, head);
        }
        #endregion

        #region Account Created HTML Template
        private string AfterCampaignApprovedTemplate(Campaign.Core.Entities.Campaign campaign)
        {
            string head = @"<head>
				<meta http-equiv='Content-Type' content='text/html; charset=UTF-8' />
				<title>Account Created</title>
    ...
    {
        Task SendEmail(SmtpClient client, string toAddress, EmailType type, Campaign.Core.Entities.Campaign campaign);
    }
}
